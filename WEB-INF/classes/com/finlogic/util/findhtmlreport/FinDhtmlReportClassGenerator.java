...
import com.finlogic.apps.finstudio.findhtmlreport.FinDhtmlReportService;
 ...
 * @author njuser
 */
public class FinDhtmlReportClassGenerator
{

    public void generateReportClass(final FinDhtmlReportDetailEntityBean dEntityBean, final FinDhtmlReportSummaryEntityBean sEntityBean) throws FileNotFoundException, ClassNotFoundException, SQLException, UnsupportedEncodingException
    {
...
//                }
                    ...
                    if (("ComboBox".equals(control[i]) || "TextLikeCombo".equals(control[i])) && dEntityBean.getFltrCmbSource() != null && "fltrCmbSrcQuery".equals(dEntityBean.getFltrCmbSource()[i]))
                    {
                        if (dEntityBean.getFltrchkForDependentCombo() != null && !dEntityBean.getFltrchkForDependentCombo()[i].equalsIgnoreCase("false") && dEntityBean.getFltrchkForDependentCombo().length > 0)
                        {
                            if (dEntityBean.getFltrCmbDependent() != null)
                            {
                                ...
                                {
                                    String getMethodName = depComboArry[i].substring(0, 1).toUpperCase(Locale.getDefault()) + depComboArry[i].substring(1).toLowerCase(Locale.getDefault());
                                    if (!depComboArry[i].equalsIgnoreCase(methodName))
                                    {
                                        ...
                                        pw.println("    {");
                                        pw.println("        StringBuilder queryMast = new StringBuilder();");
                                        String query = tmpQuery[i].split("WHERE")[0];
                                        pw.println("        queryMast.append(\"" + query + "\"); ");
                                        if (dEntityBean.getFltrRbtnMultiple()[i].equals("true") || dEntityBean.getFltrRbtnMultiple()[i].equals("N/A"))
                                        {
                                            pw.println("        if(formBean.get" + getMethodName + "()!=null && formBean.get" + getMethodName + "().length>0) ");
                                            pw.println("         { ");
                                            pw.println("          queryMast.append(\" WHERE  " + tmpQuery[i].split("WHERE")[1].split("=")[0] + " IN ( \").append(Pagging.CommonFunctions.getCommaSeperatedString(formBean.get" + getMethodName + "(),true)).append(\")\"); ");
                                            pw.println("         } ");
                                        }
                                        else
                                        {
                                            pw.println("        if(formBean.get" + getMethodName + "()!=null && !formBean.get" + getMethodName + "().equalsIgnoreCase(\"\")) ");
                                            pw.println("         { ");
                                            pw.println("          queryMast.append(\" WHERE  " + tmpQuery[i].split("WHERE")[1].split("=")[0] + " ='\").append(formBean.get" + getMethodName + "()).append(\"'\"); ");
                                            pw.println("         } ");
                                        }
    ...
    {
        FinDhtmlReportService frs = new FinDhtmlReportService();
