 ...
 * @author njuser
 */
public class FinDhtmlReportFilterGenerator
{

    public void generateFilterJsp(final FinDhtmlReportDetailEntityBean entityBean) throws FileNotFoundException, ClassNotFoundException, SQLException, UnsupportedEncodingException
    {
        ...
        for (int i = 0; i < tabIndexLen; i++)
        {
            ...
            if (control[tabIndex[i]].equals("ComboBox") || control[tabIndex[i]].equals("TextLikeCombo"))
            {
                ...
                if (entityBean.getFltrCmbDependent() != null)
                {
                    ...
                    for (int k = 0; k < depComboArry.length; k++)
                    {
                        if (depComboArry[k].equalsIgnoreCase(entityBean.getFltrTxtName()[tabIndex[i]]))
                        {
                            flag = true;
                            break;
                        }
            ...
            else if (control[tabIndex[i]].equals("Radio"))
            {
                ...
                String[] allCaption = entityBean.getFltrTxtRdoCaption()[tabIndex[i]].split("/");
                for (int j = 0; j < total; j++)
                {
                    ...
                    pw.println("                <label> " + allCaption[j] + " </label>");
                }
                pw.println("            </td>");
            }
